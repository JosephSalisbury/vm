...
func (p *virtualBoxProvider) Create(channel string, ignition ignition.Interface, cpu int, ram int) error {
	...
	vboxConfigdriveGenOut, err := exec.Command(
		"bash", "-c",
		fmt.Sprintf("cat %s | vbox-configdrive-gen > %s", ignitionPath, configDriveImagePath),
	).Output()
	if err != nil {
